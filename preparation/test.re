...
我々は3次元空間に住んでいるため、力学でも3次元空間中の運動を扱うことが多い。3次元空間中では、質点の位置を指定したり、速度や力を指定したりするのに3つの数字の組が必要になる。例えば、ある質点の座標が$(r@<b>{x,r}y,r_z)$である時、
...
とまとめて表記する。このように、いくつかの数字の組で表現される量を @<strong>{ベクトル(vector)} と呼ぶ。特に、$\mathbf{r}$ は位置ベクトルと呼ばれる。同様に速度ベクトルなどが定義される。本講義ノートでは$\mathbf{r}$のように太字で表記する。板書においては、通常のアルファベットに縦線を一本加えることで太字を表現する。



一方、ベクトルではない普通の数を @<strong>{スカラー(scalar)} と呼ぶ。スカラーの定義は追って与える。また、板書では縦ベクトルは縦に書くが、講義ノートでは紙面の都合から横に書きたいことがある。その場合は$\mathbf{r} = {}^t(r@<b>{x, r}y, r_z)$のように、ベクトルの最初に$t$をつけて表す。この$t$は転置を表し、横ベクトルを転置することで、縦ベクトルであることを表現している。
...
3つの基本的なベクトルの和にわけることができた。この基本的なベクトルを$\mathbf{e}@<b>{x,\mathbf{e\}}y,\mathbf{e}_z$と書こう。すると、



$$
\mathbf{r} = r@<b>{x \mathbf{e}x} + r@<b>{y \mathbf{e}y} + r@<b>{z \mathbf{e}z}
$$



と表記できる。この時、$\mathbf{e}@<b>{x,\mathbf{e\}}y,\mathbf{e}_z$を基底ベクトル、この式を基底ベクトルによる展開と呼ぶ。特に、
...
\end{pmatrix\}
$$
という表記は、標準基底ベクトル$\mathbf{e\}}x,\mathbf{e}@<b>{y,\mathbf{e\}}z$を採用した時のベクトル$\mathbf{r}$の展開表現である。
...
と書くべきところを、基底ベクトルを$\mathbf{e}@<b>{x,\mathbf{e\}}y,\mathbf{e}_z$と暗黙に仮定した上で括弧で略記した記法である。



さて、今は$\mathbf{e}@<b>{x$などが簡単な形をしているため、$\mathbf{r\}$を$\mathbf{e\}}x$などで展開した時の係数が簡単に求まった。しかし、一般のベクトルを基底ベクトルにした場合、その展開係数を求めるのは面倒である。そこで、ベクトルの内積により展開係数を求める。



基底ベクトル$\mathbf{e}@<b>{x,\mathbf{e\}}y,\mathbf{e}_z$は、要素が縦に並んでいる縦ベクトルであった。これらの対になるベクトルとして、以下の横ベクトルを定義する。
...
いちいち座標を$x,y,z$と書いていると、特に和を取る時に不便なので、以下$\mathbf{e}@<b>{x,\mathbf{e\}}y,\mathbf{e}@<b>{z$を、それぞれ$\mathbf{e\}}1,\mathbf{e}@<b>{2,\mathbf{e\}}3$と表記しよう。相方となる基底$\mathbf{e}^1,\mathbf{e}^2,\mathbf{e}^3$も同様である。この表記を用いると、先程の内積は以下のようにまとめて書ける。



$$
(\mathbf{e}^i, \mathbf{e}@<b>{j) = \delta}{i,j}
$$
...
一般に、あるベクトルと、内積をとる相手のベクトルの住む空間は異なる。自分が住む空間から見て、内積を取る相手が住む空間を @<strong>{双対空間(dual space)} と呼ぶ。双対空間はお互い様であり、縦ベクトルから見れば横ベクトルが住む空間が双対空間だが、横ベクトルから見れば縦ベクトルが住む空間が双対空間である。また、ある基底と内積をとる相方の基底のことを双対基底と呼ぶ。双対基底は、双対空間に住んでいる基底である。



ここまではベクトルが住む空間がわかりやすいように、住む世界を添え字の上下で区別したが、２つの空間を区別する必要がない場合は２つのベクトル$\mathbf{a} = {}^t(a@<b>{1, a}2, a@<b>{3)$と$\mathbf{b\} = {\}^t(b}1, b@<b>{2, b}3)$の内積を
...
$$
\begin{aligned}
\mathbf{r} \cdot \mathbf{e@<b>{x\} &= r}x \
\mathbf{r} \cdot \mathbf{e@<b>{y\} &= r}y \
\mathbf{r} \cdot \mathbf{e@<b>{z\} &= r}z
\end{aligned}
$$
...
さて、ある基底$\mathbf{e}@<b>{1, \mathbf{e\}}2, \mathbf{e}@<b>{3$で表現されたベクトル$\mathbf{r\} = {\}^t(r}1, r@<b>{2, r}3)$について、別の基底$\mathbf{e}'@<b>{1, \mathbf{e\}'}2, \mathbf{e}'@<b>{3$で展開した時の表現$\mathbf{r\} = {\}^t(r'}1, r'@<b>{2, r'}3)$を求めたい。つまり、以下のように、ある基底での展開係数がわかっている時、別の基底で表現した場合の展開係数を求めたい。



$$
\begin{aligned}
\mathbf{r} &= r@<b>{1 \mathbf{e\}}1  +r@<b>{1 \mathbf{e\}}2+r@<b>{3 \mathbf{e\}}3\
&= r'@<b>{1 \mathbf{e\}'}1  +r'@<b>{1 \mathbf{e\}'}2+r'@<b>{3 \mathbf{e\}'}3\
\end{aligned}
$$
...
例えば、基底ベクトル$\mathbf{e}'@<b>{1$に対する成分$r'}1$を知りたければ、$\mathbf{e}'_1$と$\mathbf{r}$との内積を取ればよい。



$$
r'@<b>{1 = \mathbf{e\}'}1 \cdot \mathbf{r}
$$



さて、$\mathbf{r}$を基底$\mathbf{e}@<b>{i$で展開しておき、左から別の基底$\mathbf{e\}'}j$との内積を取ろう。



$$
\begin{aligned}
r'@<b>{j &= \mathbf{e\}'}j \cdot \mathbf{r}  \
&= \sum@<b>{i r}i \mathbf{e}'@<b>{j \cdot  \mathbf{e\}}i
\end{aligned}
$$



この式は「あるベクトル$\mathbf{r}$について、ある基底$\mathbf{e}@<b>{i$での展開係数がわかっている場合、別の基底$\mathbf{e\}'}j$での展開係数を知るには、基底同士の内積 $\mathbf{e}'@<b>{j \cdot  \mathbf{e\}}i$ がわかっていればいれば良い」ということを意味する。
...
基底$\mathbf{e}@<b>{1$と、その相方とのテンソル積 $\mathbf{e\}^1 \otimes \mathbf{e\}}1$を以下のように定義する。
...
A &=
\begin{pmatrix}
A^1@<b>{1 & A^2}1 & A^3@<b>{1 \
A^1}2 & A^2@<b>{2 & A^3}2 \
A^1@<b>{3 & A^2}3 & A^3@<b>{3
\end{pmatrix\}
= \sum}{i=1}^{3} \sum@<b>{{j=1\}^{3\} A^i}j \mathbf{e}^i \otimes \mathbf{e}_j
\end{aligned}
$$



さて、多くの場合において、和の範囲は自明である。例えば$i$が軸に関するものであれば1から3までであろうし、$i$が粒子番号であれば$1$から$N$まで走るであろう。その場合にいちいち和記号$\Sigma$を書くのは面倒である。そこで、同じ添字が２つ現れたら、その添字について和を取る、という約束をする。これを @<strong>{アインシュタイン規約(Einstein notation)} と呼ぶ。
...
A^1@<b>{3 & A^2}3 & A^3@<b>{3
\end{pmatrix\}
= A^i}j \mathbf{e}^i \otimes \mathbf{e}_j
\end{aligned}
$$



ベクトルや行列を要素表現した時の添字の数を「足」と呼ぶ。ベクトルのテンソル積を考えることで足を増やすことができる。ベクトルは足が1本であり、行列は足が2本である。ベクトルのテンソル積により作られた世界は、元になったベクトルよりも広い世界になっている。例えば3次元縦ベクトルと横ベクトルのテンソル積は、3行3列の行列、すなわち9次元の空間になっている。このベクトルや行列をまとめて @<strong>{テンソル(Tensor)} と呼ぶ。足の本数、つまり添字の数をテンソルの階数と呼ぶ。ベクトルは1階のテンソル、行列は2階のテンソルである。テンソル積により、いくらでも高階のテンソルを考えることができる。
...
同様に、行列$A$と、縦ベクトルの基底$\mathbf{e}@<b>{k$の内積$(A, \mathbf{e\}}k)$を考えよう。行列$A$は、縦ベクトルと横ベクトルのテンソル積で展開できるのであるから、



$$
(A, \mathbf{e}@<b>{k) = (A^i}j \mathbf{e}^i \otimes \mathbf{e}@<b>{j, \mathbf{e\}}k)
$$
...
$$
(\mathbf{e}^i, \mathbf{e}@<b>{k) = \delta}{ik}
$$
...
$$
A = A@<b>{{ij\} \mathbf{e\}}i \otimes \mathbf{e}_j
$$
...
などと分数の形で表記する。この表記を @<strong>{ライプニッツ記法(Leibniz's notation)} と呼ぶ@<fn>{1}。
...
とプライム記号を増やして表記する。このような表記を @<strong>{ラグランジュ記法(Lagrange's notation)} と呼ぶ。@<fn>{2}
...
これを @<strong>{ニュートン記法(Newton's notation)} と呼ぶ。@<fn>{3}
...
$y$に関する偏微分係数も同様に定義する。以後、$x$に関する偏微分演算子$\partial/\partial@<b>{x$を、$\partial}x$のように略記することがある。
...
このように書ける時$f$は全微分可能であると言い、この表式を$f$の@<strong>{全微分}と呼ぶ。
...
$$
\begin{aligned}
\frac{\partial f}{\partial x} &= \frac{\partial f}{\partial x} \underbrace{\frac{\partial x}{\partial x}}@<b>{{=1\}
+ \frac{\partial f\}{\partial y\} \underbrace{\frac{\partial y\}{\partial x\}\}}{=0} \
&= \frac{\partial f}{\partial x}
\end{aligned}
$$
...
$$
\mathbf{f} = \frac{\partial f}{\partial x} \mathbf{e}@<b>{x +
\frac{\partial f\}{\partial y\} \mathbf{e\}}y
$$
...
となる。以上から、$df$をベクトル、$dx$や$dy$を基底のように扱うことができることがわかる。さらに、$dx$をベクトルと見た時、その相方となるベクトルが$\partial@<b>{x$であることもわかる。すなわち、$dx, dy, dz$と$\partial}x, \partial@<b>{y, \partial}z$の間には、自然な内積、
...
が定義される。このように、$dx, dy$を縦ベクトル、$\partial@<b>{x, \partial}y$を横ベクトルだと思うことができ、それらの間に内積を定義することができた。このように考えると、$dx, dy$が住む世界と$\partial@<b>{x, \partial}y$が住む世界は双対空間である。
 ...
 * 参考：@<href>{http://www.tanomura.com/research/papers/tanomura_2018_dash_prime_addenda.pdf,a' の英語における読みの歴史に関する覚書  ─「ダッシュ、プライム」余論─ 田野村忠温}


//footnote[1][ゴッドフリート・ライプニッツ(Gottfried Wilhelm Leibniz, 1646-1716)に因む。積分記号なども彼の発明である。]

//footnote[2][ジョセフ＝ルイ・ラグランジュ(Joseph-Louis Lagrange, 1736-1813)に因む。解析力学の創始者の一人である。]

//footnote[3][アイザック・ニュートン(1642–1727)に因む。ニュートン力学を構築した。微積分の研究、万有引力の法則や、光学の研究でも有名。]
