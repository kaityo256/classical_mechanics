...
$F=ma$という式について、さらに別の見方をしてみよう。$ma$を左辺に移項して、こんな式を作ってみる。
...
この式は、紐の張力によって、常に物体の進行方向から90度傾いた方向に力$F$が作用しており、その結果として加速度$v^2/r$が生じている、ということを意味している。
...
ニュートンの運動方程式、$F=ma$から始めよう。$F$が力、$m$が質量、$a$が加速度である。いま、座標を$x$とし、力が保存力であるとして、$F=-V'(x)$と書ける場合を考えると、運動方程式は以下のようになる。
