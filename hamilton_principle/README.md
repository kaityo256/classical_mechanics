...
この量を **作用 (action)**、この積分を作用積分と呼ぶ。この作用積分に対して、

$$
\delta I = 0
$$

という関係式を満たすことを要求すると、その結果として運動方程式の解$(v(t),x(t))$を得る。これを **ハミルトンの原理 (Hamilton's principle)** と呼ぶ。この量$L$を **ラグランジアン (Lagrangian)** と呼ぶ。保存力に対するポテンシャル関数と対比し、運動ポテンシャルと呼ぶこともある。
...
なお、ここでは発見法的にラグランジアンと運動方程式の関係を導いたが、より根源的には、作用積分に対して変分原理を適用することでラグランジアンからオイラー・ラグランジュの式が導かれる。変分原理については次章で扱う。
