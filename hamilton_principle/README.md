...
運動方程式から、ダランベールの原理を用いてラグランジアンを導いた。逆に、ラグランジアンから運動方程式を作ることができる。その前に多自由度系における運動方程式とラグランジアンの関係を見ておこう。

二次元空間において、質量$m$の質点がポテンシャル$U(x,y)$の影響を受けて運動しているとしよう。運動方程式は、

$$
\begin{aligned}
m \frac{d \dot{x}}{dt} &= -\frac{\partial U}{\partial x} \\
m \frac{d \dot{y}}{dt} &= -\frac{\partial U}{\partial y}
\end{aligned}
$$

で記述できる。ここで、2つの自由度$x,y$方向にかかる力が、ただ一つのスカラー関数$U$で記述できていることに注意しよう。右辺が同じスカラー関数から導出できてるのだから、左辺も一緒に導出したくなる。そのために、運動エネルギーの「ポテンシャル」を考えよう。この系の運動エネルギーは
...
で与えられ、これが運動方程式の左辺を導くポテンシャルの役割を果たす。運動エネルギー$K$と、ポテンシャルエネルギーの差から

$$
L(x, y, \dot{x}, \dot{y}) = \frac{1}{2}m(\dot{x}^2+\dot{y}^2) - U(x,y)
$$

によってラグランジアンを定義する。ここで$x, y, \dot{x}, \dot{y}$が全て独立だと思って、$x$や$\dot{x}$でラグランジアンを偏微分しよう。

$$
\begin{aligned}
\frac{\partial L}{\partial x} &= -\frac{\partial U}{\partial x} \\
\frac{\partial L}{\partial \dot{x}} &= m \dot{x} \\
\frac{d}{dt}\left(\frac{\partial L}{\partial \dot{x}} \right) &= m \frac{d \dot{x}}{dt}
\end{aligned}
$$
...
全く同様にして、自由度$y$についての運動方程式も

$$
\frac{d}{dt}\left(\frac{\partial L}{\partial \dot{y}} \right) = \frac{\partial L}{\partial y}
$$

で与えられ、これは$y$座標が従うニュートンの運動方程式と等価である。こうして、2つの自由度$x,y$が従う2本の運動方程式が、ただ1つのスカラー関数$L(x, y, \dot{x}, \dot{y})$から導かれた。

以上の議論を一般化し、$N$自由度系を考える。この系の配位空間は、$N$個の一般化座標$q^1, q^2, \cdots, q^N$で記述されている。もしこの系のラグランジアン$L(q^1, q^2, \cdots, q^N, \dot{q}^1,\dot{q}^2,\cdots, \dot{q}^N)$が与えられたならば、この系の$N$本の運動方程式は、
