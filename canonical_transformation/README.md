...
もともとニュートンの運動方程式からラグランジュ形式の力学に移ったのは、変数変換に強くなるから、という目的があった。ハミルトン形式の力学に移ることで、正準変換という、ラグランジュ形式の力学よりもさらに広い形の変数変換を許容するようになる。さらに、単一のスカラー関数である母関数から正準変換が生成されることを見た。今後、正準変換は単なる変数変換という枠組みを超え、空間並進や回転、時間発展すら正準変換の一種であること、さらに正準変換と保存則が密接に関係していることを見ていく。
