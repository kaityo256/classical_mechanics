...
繰り返しになるが、系の運動はハミルトニアンというスカラー関数一つだけから全て決まる。したがって、ハミルトニアンには、系の運動の情報、特に対称性や保存則についての情報が含まれている。そのうち、最も簡単なものは循環座標であり、ハミルトニアンが、ある一般化座標$q^i$に依存しなければ、対応する正準共役な運動量$p_i$が保存量となる。この性質をもっと一般化して調べたい。
...
と、$O(\varepsilon)$だけズレた母関数を考える。この母関数による正準変換は以下のようになる。
