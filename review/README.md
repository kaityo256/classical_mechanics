...
である。ここから、バネ定数$k$の次元を求めてみよう。長さ$x$の次元は$[L]$であるから、力の次元がわかれば、バネ定数の次元もわかる。力の次元は、ニュートンの運動方程式、
...
バネ定数の次元がわかると、バネの周期の質量依存性が推定できる。バネ定数$k$に質量$m$の質点が繋がれている時、質点を釣り合いの位置からずらして運動させると周期運動をするであろう。その周期を$T$とする。周期$T$がバネ定数や質量にどのように依存するか知りたい。釣り合いの位置を原点、初期位置を$x_0$とする時、時刻$t$における位置$x(t)$は運動方程式を解くことで
...
と求められる。しかし、次元解析を使うと、運動方程式を解くことなく周期の依存性がわかる。
