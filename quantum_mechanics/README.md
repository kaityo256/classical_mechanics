...
を用いると、平面波展開は以下のように書き直すことができる(なぜこのように書けるかは本書では)。
...
これは、括弧(bracket)の左側 $\left< \right.$ をブラ(bra)、右側$\left. \right>$をケットと分解する、Diracが考案した記法である。それぞれブラベクトル、ケットベクトルと呼ぶが、ブラ、ケットと略すことも多い。この表記を用いると、波動関数がエネルギー演算子の固有関数であることがわかりやすい。
...
解析力学において、ポアソン括弧により運動量と座標には以下のような関係が成り立っていた。

$$
\left\{q^i, p_j \right\} = \delta_{}
$$
