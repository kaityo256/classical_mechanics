...
まず、自由変数を取り直す、ということの意味について考える。簡単のために、一変数関数$y(x)$を考えよう。この関数の全微分は

$$
dy = \frac{dy}{dx} dx
$$

と記述できる。また、右辺に$dx$しか現れないのは、この関数$y$が$x$にのみ依存する関数であることを表している。すなわち、$x$が自由変数であり、$dy\dx$がその微係数である。さて、ここで、自由変数と微係数を入れ替えることを考える。つまり、
...
もともとラグランジアンは$q$と$\dot{q}$の関数であり、$L(q,\dot{q})$と書けた。従って、その全微分は
