...
$$
m \ddot{x^i} = mT^i_j \ddot{\tilde{x}^j}
$$
...
$$
m\ddot{\tilde{x}^j} = m T^i_j \ddot{x^i}
$$
...
従って、$m \ddot{x}^j - F_j = 0$が成り立つなら、$m\ddot{\tilde{x}^i} - \tilde{F}_i$=0$も成り立つ。すなわち、ニュートンの運動方程式は直交変換$T$に対して共変である。
