...
$$
m\ddot{\tilde{x}^i} = m T^i_j \ddot{x^i}
$$
...
$$
\begin{aligned}
m\ddot{\tilde{x}^i} - \tilde{F}_i &= T^j_i (m \ddot{x}^j - F_j)
\end{aligned}
$$

従って、$m \ddot{x}^j - F_j = 0$ が成り立つなら、$m\ddot{\tilde{x}^i} - \tilde{F}_i$=0$ も成り立つ。すなわち、ニュートンの運動方程式は直交変換に対して共変である。このように、何かが変換に対して共変であることを示したいとき、成り立つべき式がゼロとなるような形を作っておき、右辺がゼロなので左辺もゼロ、という論法をよく使うので気に留めておくとよい。
...
つまり、$m\ddot{x} = F$が成り立っていても、$m\ddot{\tilde{x}} = \tilde{F}$は成立しない。このように、直交変換でない変換についてはニュートンの運動方程式は共変ではない。極座標への変換も、ニュートンの運動方程式が共変でない座標変換である。極座標はよく使うので、運動方程式が共変でないといろいろ面倒だ。そこで、より広いクラスの変換に対して共変な運動方程式を使いたくなる。それが後に学ぶラグランジュ形式や、ハミルトン形式の運動方程式である。
...
リンゴが机の上に乗って静止している系には、3つの物体の間に4つの力が働いている。まずはリンゴが机を押す力と、その反作用である机がリンゴを押す力である。また、リンゴは地球と万有引力によりお互いに引っ張りあっている。こちらも地球がリンゴを引っ張る力と、リンゴが地球を引っ張る力が同じ大きさ、逆向きのベクトルで釣り合っている。
