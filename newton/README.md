...
<!--- abstract --->
本格的に解析力学を学ぶ前に、ニュートンの運動方程式の性質について確認する。まず、ニュートン力学について復習し、次にニュートンの運動方程式がガリレイ変換に対する共変性を持っていることを確認する。そのために、反変ベクトル、共変ベクトルの区別について学び、その後、座標変換に対する運動方程式の共変性について調べる。
<!--- end --->
...
本書では、質点の運動のみを扱うことにする。なお、有限の大きさを持つ物体の運動も扱うが、その物体の回転などを扱わないため、質点の運動として記述できる。
...
地図を思い浮かべて欲しい。地図には等高線が書いてあり、その場所での標高がわかるようになっている。この地図を使って、現在自分がいる地点の標高を調べたとしよう。さて、目的地に向かうため、道の方向に合わせて地図の向きを回転させたとしよう。これは座標変換したことに対応するが、座標変換しても現時点の高さは変わらない。このように、座標変換により変換を受けない量を **スカラー**(scalar) と定義する。
...
と結びついているとする。この時、この変換の前後のベクトルの成分$u^i$と$\tilde{u}^i$の間の関係が、基底の変換と逆であること、すなわち$u^i$と$\tilde{u}^i$が反変ベクトルであることを証明する。

ベクトル$\mathbf{u}$を新しい基底$\{\tilde{\mathbf{e}}_i\}$で展開した式に、基底の変換の関係を代入する。
...
チルダが現れる項が左右逆になっていることがわかる。$\mathbf{u}$は基底の変換と反対の変換を受ける反変ベクトルとなっている。
...
$$
\tilde{\mathbf{e}}^j = M_i^j \mathbf{e}^i
$$

の両辺に$T_j^k$をかけて和をとると、

$$
\begin{aligned}
T_j^k \tilde{\mathbf{e}}^j  &= T_j^k M_i^j \mathbf{e}^i\\
&= \delta_i^k\mathbf{e}^i\\
&= \mathbf{e}^k
\end{aligned}
$$
...
$$
\mathbf{e}^k = T_j^k \tilde{\mathbf{e}}^j
$$

と、縦ベクトルの変換とは逆向きの変換になっており、反変ベクトルであることがわかる。
...
のように、基底の変換とベクトルの変換のチルダのある方が同じ側に来ることがわかる。このような変換をするベクトルを**共変ベクトル** (covariant vector)と呼ぶ。

さて、もともとの基底の変換則は以下で与えられていた。
...
であったから、${}^tT = M$である。先程$M=T^{-1}$であることを証明したので、${}^tT = T^{-1}$、すなわち変換行列$T$は転置を取ると逆行列になることがわかる。この性質を持つ行列を直交行列、この行列による変換を直交変換と呼ぶ。このように、正規直交基底から正規直交基底へ変換する行列は直交行列となる。
...
これは、点$P$における$(-\theta)$回転に他ならない。図を見れば、確かに$(dr, d\theta)$が張る座標系を$-\theta$だけ回転させたものが$(dx, dy)$が張る座標系になっていることがわかる。このように、どのような非線形変換であっても、適当な点の近傍で展開することで線形変換とすることができる。
...
いま、$(x^1, x^2)$で張られる空間に、スカラー関数$f(x^1, x^2)$があるとしよう。この関数の勾配が、変数変換$(x^1, x^2) \rightarrow (\tilde{x}^1, \tilde{x}^2)$でどのように変換されるか見てみよう。合成関数の偏微分の公式から、
...
$$
T^i_j = \frac{\partial x^i}{\partial \tilde{x}^j}
$$
...
従って、$m d^2{x}^j/dt^2 - F_j = 0$ が成り立つなら、$m d^2\tilde{x}^i/dt^2 - \tilde{F}_i=0$ も成り立つ。すなわち、ニュートンの運動方程式は直交変換に対して共変である。このように、何かが変換に対して共変であることを示したいとき、成り立つべき式がゼロとなるような形を作っておき、右辺がゼロなので左辺もゼロ、という論法をよく使うので気に留めておくとよい。

ここで、 共変性は**不変性**(invariance)とは異なることに注意したい。例えばスカラー量は座標変換で値そのものを変えないので不変である。一方、運動方程式は、座標変換によりベクトルの値は変更を受けるが、形を変えない。従って、線形変換で移ることができる異なる座標系に住む人にとって、同じ形の法則が成り立つ。これを共変であると呼ぶ。
...
ただし$\mathbf{u}$は定ベクトルである。この変換を **ガリレイ変換**(Galilean transformation) と呼ぶ。
...
ニュートンの運動方程式は直交変換及びガリレイ変換に対して共変である。ガリレイ変換は、ある慣性系に対して加速度を持たない別の慣性系への座標変換を表現している。ニュートンの運動方程式がガリレイ変換により共変であるとは、ニュートンの運動方程式がすべての慣性系において同じ形をしていることを意味する。これは逆に、「どの慣性系から見ても同じ運動法則が成り立つようにニュートンの運動方程式が構成されている」と考えることもできる。「どの慣性系から見ても同じ運動法則が成り立つ」という要請をガリレイの相対性原理と呼ぶ。一方、電磁場の基礎方程式であるマクスウェルの方程式は、ガリレイ変換に対して共変ではないが、ローレンツ変換に対して共変性を持つ。ローレンツ変換において、物体の速度が光速に対して無視できるという近似をするとガリレイ変換に帰着する。アインシュタインは運動方程式やマクスウェル方程式を含む全ての物理法則が、あらゆる慣性系から同じように見えなければならないという原理を要請することで、特殊相対性理論を導いた。この要請を特殊相対性原理と呼ぶ。

ニュートンの運動方程式が直交変換及びガリレイ変換で共変であることを見たが、一般の変換に対しては共変ではない。簡単な例として、一次元空間の保存力による運動

$$
m \ddot{x} = F = - \frac{dU}{dx}
$$

を考えよう。いま、座標を定数倍する$\tilde{x} = a x$という変換を考える。運動方程式の左辺は

$$
m \ddot{\tilde{x}} = a m \ddot{x}
$$
...
つまり、$m\ddot{x} = F$が成り立っていても、$m\ddot{\tilde{x}} = \tilde{F}$は成立しない。このように、直交変換でない変換についてはニュートンの運動方程式は共変ではない。極座標への変換も、ニュートンの運動方程式が共変でない座標変換の例である。極座標はよく使うので、運動方程式が共変でないといろいろ面倒だ。そこで、より広いクラスの変換に対して共変な運動方程式を使いたくなる。それが後に学ぶラグランジュ形式や、ハミルトン形式の運動方程式である。
...
$$
-kx = - \frac{d}{dx} \left( \frac{1}{2}kx^2\right)
$$
...
このうち、右辺第一項を **運動エネルギー**(kinetic energy) と呼び、$K$で表すことが多い。左辺第二項を **ポテンシャルエネルギー**(potential energy) と呼び、$U$で表すことが多い。一般的に、物体に働く力が全てポテンシャルから引き出される場合、運動エネルギーとポテンシャルエネルギーの和である全エネルギーが必ず保存量となる。
...
であり、ここから速度を求めることができる。当たり前だと思うであろうが、ここで斜面の形状の情報を全く使っていないことに注意して欲しい。斜面の形状を変えると、落下を開始してから高さ$h$だけ落ちるまでの時間が変化する。にもかかわらず、斜面がどのような形状であったとしても、最終的な速度は高さの差にしか依存しないことをエネルギー保存則は主張する。
...
$$
m \ddot{x}_B = U'(r) \frac{(x_A-x_B)}{r}
$$

となる。$y, z$方向も同様である。従って、質点A, B間に働く力は、大きさは同じで向きは逆向きとなり、作用・反作用の法則が成り立っていることがわかる。このように、相対座標のみに依存するポテンシャルによる相互作用は、作用・反作用の法則を自動的に満たす。
...
![リンゴに働く力](fig/apple.png)

机の上にリンゴが乗っている。リンゴは重力により下向きの力を受けている。では、リンゴに働く重力の反作用は何か？

正解は、リンゴが地球を引っ張る力である。作用反作用の法則が「2つの質点の間に力が働く時、両者の大きさは等しく、向きは逆向きとなる」であり、リンゴに働く重力と抗力は大きさが等しく、向きが逆向きであることから、うっかり、机からの抗力と答えてしまいがちである。

リンゴが机の上に乗って静止している系には、3つの物体の間に4つの力が働いている。まずは「リンゴが机を押す力」と、その反作用である「机がリンゴを押す力」である。また、リンゴは地球と万有引力によりお互いに引っ張りあっている。こちらも「地球がリンゴを引っ張る力」と、「リンゴが地球を引っ張る力」が同じ大きさ、逆向きのベクトルで釣り合っている。

リンゴが静止している以上、リンゴに働く力は釣り合っている。リンゴには重力と抗力しか働いておらず、力が釣り合っているのであるから、それらは同じ大きさで逆向きでなければならない。しかし、これは力の釣り合いである。作用・反作用の法則と混同してはならない。作用・反作用の法則は、あくまでも力を及ぼし合う **二つの物体** の間に働く関係であり、お互いに同じ大きさ、逆向きの力が働いているが、物体は静止している必要はない。例えば物体を押して加速している場合も、押している側と加速している物体の間には作用・反作用が成り立っており、かつ物体は静止していないから力は釣り合っていない。

では、これを力の釣り合いと言ってはならないかというとそうではない。質量と加速度の積は力の次元を持つため、$F=ma$と等式で結ぶことができる。すると、右辺も力と同一視できるため、左辺に移項して力の釣り合いの式とみなすことができる。これは後にダランベールの原理として学ぶ。
