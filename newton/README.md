...
従って、$m \ddot{x}^j - F_j = 0$が成り立つなら、$m\ddot{\tilde{x}^i} - \tilde{F}_i$=0$も成り立つ。すなわち、ニュートンの運動方程式は直交変換$T$に対して共変である。このように、何かが変換に対して共変であることを示したいとき、成り立つべき式がゼロとなるような形を作っておき、右辺がゼロなので左辺もゼロ、という論法をよく使うので気に留めておくとよい。
