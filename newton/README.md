...
ここで、 **共変性**(covaiance) は **不変性**(invariance) とは異なることに注意したい。例えばスカラー量は座標変換で値そのものを変えないので不変である。一方、運動方程式は、座標変換によりベクトルの値は変更を受けるが、形を変えない。従って、線形変換で移ることができる異なる座標系に住む人にとって、同じ形の法則が成り立つ。これを共変であると呼ぶ。
