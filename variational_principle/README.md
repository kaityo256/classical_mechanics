...
まずは簡単な例からはじめよう。図のA地点からB地点まで行きたい。ただし、A地点は草原で歩きやすく、B地点は歩きにくい沼地にある。草原と沼地が直線で接しており、A地点、$B$地点から境界までの距離はそれぞれ$h_A$、$h_B$、A地点とB地点の横方向の距離は$L$であるとする。A地点からB地点まで最短で到達できる経路はどのようになるか？
...
この問題を考えるため、直線AXの長さを$l_A$、XBの長さを$l_B$とする。A地点が$y$軸上に、境目が$x$軸となるような座標系を採用する。するとA地点の座標は$(0,h_A)$、B地点の座標は($L$, $-h_B$)となる。草原を歩く時の速度は$v_A$、沼地を歩く時の速度を$v_B$とする($v_A>v_B$)。A地点から草原と沼地の境目の点$X(x,0)$までまっすぐ歩き、そこからB地点までまっすぐ歩くことにする。AXの距離は$\sqrt{x^2+l_A^2}$、XBの距離は$\sqrt{(L-x)^2+l_B^2}$であり、それぞれ$v_A$、$v_B$の速度で歩くのであるから、全体を歩くのにかかる時間$T(x)$は
