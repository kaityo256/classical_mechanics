...
$$
\begin{aligned}
F[\exp(x)] &= \int_a^b \mathrm{e}^x dx\\
&= \left[ \mathrm{e}^x \right]_a^b \\
&= \mathrm{e}^b - \mathrm{e}^a
\end{aligned}
$$

となる。このように、汎関数とは、何かしら関数を放り込むと、対応する実数を返すものだ。

さて、ある微少量$\varepsilon$に対して
...
同様に、関数$f(x)$に対して、何か適当な関数$\phi(x)$を考え、$\varepsilon$を微小量として

$$
F[f+\varepsilon \phi] = F[f] +  \frac{\delta F}{\delta f} \varepsilon \phi + O(\varepsilon^2)
$$

と表せる時、$\delta F/\delta f$を、$F$の汎関数微分、もしくは第一変分と呼ぶ。
