...
<!--- abstract --->
本章では、変数変換に対する運動方程式の変換について学ぶ。解析力学はニュートン力学を再定式化したものであるから、本質的には同じ問題を別の表示に書き換えているのに過ぎない。しかし、別の形式で書き直すことで見通しがよくなり、ニュートン力学では見えなかった世界が見えてくる。まずは、ラグランジュ形式の力学が、ニュートン力学よりも広いタイプの変数変換に対して「見通しが良い」ことを見てみよう。
<!--- end --->
